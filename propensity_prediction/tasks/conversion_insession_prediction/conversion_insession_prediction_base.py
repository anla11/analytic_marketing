...
from model import BinaryClassification_Evaluation, Binarize

from propensity_prediction.tasks.base import Model_Base, EnsembleModel_Base, Task_Base

class Conversion_InSession_Base(Model_Base):
	def __init__(self, context, model_config):
		...
		self.training_constant['conversion_rate'] = None
	...
	def _train(self, X, y, training_batchsize = 100):
		if (self.training_constant['conversion_rate'] is None):
			if y is None:
				print ('No values of labels')
			self.training_constant['conversion_rate'] = np.mean(y)
	...
	def _predict(self, X, predicting_batchsize = 100):
		...
		if (self.binarize.binarize_config['method'] == 'gettop') and (self.binarize.binarize_config['ntop'] is None):
			ntop = int(len(X)*self.training_constant['conversion_rate'])
...
class Conversion_InSession_EnsembleBase(EnsembleModel_Base, Conversion_InSession_Base):
	def __init__(self, context, model_config):
		Conversion_InSession_Base.__init__(self, context, model_config)
	...
	def prepare_npdata(self, data_df, get_label=True, is_tensor=False, mode = 'training'):
		...
		if (self.get_trainingconstant('conversion_rate') is None) and (mode == 'training'):
			if y is None:
				print ('No values of labels')
			self.update_trainingconstant('conversion_rate', np.mean(y))
		if is_tensor:
	...
	def predict(self, data_df, predicting_batchsize = 100):
		...
		if (self.binarize.binarize_config['method']=='gettop') and (self.binarize.binarize_config['ntop'] is None):
			ntop = int(len(data_df)*self.get_trainingconstant('conversion_rate'))
			self.binarize.update_config('ntop', ntop)
...
class Conversion_InSession_TaskBase(Conversion_InSession_EnsembleBase, Task_Base):
	def __init__(self, context, model_config):
		super().__init__(context, model_config)
		self._create_sub_models(model_config, Conversion_InSession_Base, passing_config = ['binarize_config'])